...
class Auth extends MY_Controller {
    ...
    public function all($row  = NULL)
    {
        ...
        $config['per_page'] = $row;
        $config['base_url'] = base_url().'/auth/all/'.$row.'/';
        ...
        $this->generateCsrf();
        $this->render('admin/member/index', $data);
	}
	...
	public function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
                ...
                $this->load->model(array('users_membership_m'));
                $membership = $this->db->from('users_membership')
                        ->where(array(
                            'user_id' => $this->ion_auth->get_current_user_id(),
                            'end >=' => date('Y-m-d', time())
                        ))
                        ->get()->row();
                $this->session->set_userdata('membership', $membership->membership_id);
                $reseller = $this->db->from('reseller')
                        ->where(array(
                            'user_id' => $this->ion_auth->get_current_user_id(),
                        ))
                        ->get()->row();
                $this->session->set_userdata('reseller', $reseller->id);
				//if logged in is admin
				if ($this->ion_auth->is_admin()) {
					redirect('dashboard', 'refresh');
				}elseif ($this->ion_auth->in_group(2)) {
                    if($this->session->userdata('product')){
                        $this->load->model(array('carts_m', 'cart_details_m'));
                        ...
                        if (!$query || is_null($query) || empty($query)) {
                            ...
                            if ($insert === FALSE) {
                                //TODO MESSAGE
                                dump('terjadi kesalahan saat insert cart baru untuk member yang sedang login');
                            }
                        }
                        ...
                        foreach ($products as $key => $product) {
                            ...
                            $product_id = $this->products_m->getProductIdByCode($product);
	...
	public function change_password()
	{
		...
		else
		{
			...
			if ($change)
			{
                ...
                $this->message('Password Anda Berhasil Diganti. Silahkan Masuk Ulang.');
				$this->logout();
			}
			else
			{
