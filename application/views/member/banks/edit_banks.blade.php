...
<div class="row">
	<div class="breadcrumbDiv col-lg-12">
		<ul class="breadcrumb">
			...
			<li class="active">Ubah Rekening</li>
		</ul>
	</div>
</div>
<!--/.row-->
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-7">
		...
		<h1 class="section-title-inner"><span><i class="fa fa-map-marker"></i> Ubah Data Rekening  </span></h1>
		...
		<div class="row userInfo">
			...
			<form action="{{ site_url('member/rekening/update') }}" method="post">
			{{ $csrf }} 
					...
					<div class="form-group">
						...
						name="bank_id"> 
							@foreach($banks as $bank)
								<option {{ ($bank->id == $data->bank_id) ? 'selected' : '' }} value="{{ $bank->id }}">{{ $bank->name }}</option> 
							@endforeach
						</select>
					</div>
					<div class="form-group">
						<label for="account_name">Nama Rekening <sup>*</sup> </label>
						<input value="{{ $data->account_name }}" name="account_name" type="text" class="form-control" id="account_name"
						placeholder="Nama Rekening">
					</div>
					<div class="form-group">
						<label for="account_behalf">Atas Nama <sup>*</sup> </label>
						<input value="{{ $data->account_behalf }}" name="account_behalf" type="text" class="form-control" id="account_behalf"
						placeholder="Nama Rekening">
					</div>
					<div class="form-group required">
						<label for="account_number">Nomor Rekening <sup>*</sup> </label>
						<input value="{{ $data->account_number }}" name="account_number" required type="text" class="form-control"
						id="account_number" placeholder="Nama Awal">
					</div> 
					<div class="form-group">
						<button class="btn btn-primary" type="submit"><i class="fa fa-send-o"></i> Ubah</button>
